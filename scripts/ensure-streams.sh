...
run_nats() {
  sudo docker compose -f "$COMPOSE" --env-file "$ENV_FILE" run --rm \
    -e NATS_URL="$NATS_URL" \
    -e NATS_RAW_INTEL_STREAM_MAX_AGE="$NATS_RAW_INTEL_STREAM_MAX_AGE" \
    -e NATS_STRUCTURED_INTEL_STREAM_MAX_AGE="$NATS_STRUCTURED_INTEL_STREAM_MAX_AGE" \
    -e NATS_STRUCTURED_INTEL_STREAM_DUPLICATE_WINDOW="$NATS_STRUCTURED_INTEL_STREAM_DUPLICATE_WINDOW" \
    -e NATS_INTEL_CANDIDATE_STREAM_MAX_AGE="$NATS_INTEL_CANDIDATE_STREAM_MAX_AGE" \
    -e NATS_INTEL_CANDIDATE_STREAM_DUPLICATE_WINDOW="$NATS_INTEL_CANDIDATE_STREAM_DUPLICATE_WINDOW" \
    nats-init "$@"
}
